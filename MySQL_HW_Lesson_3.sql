...
CREATE TABLE blacklist (
 ...
 blacklist_id BIGINT UNSIGNED NOT NULL PRIMARY KEY (bl_user_id, blacklist_id) -- (В чёрный список нельзя поместить пользователя более 1 раза)
 FOREIGN KEY (bl_user_id) REFERENCES users(id) -- юзер из черного списка забанен только у `Пети`
);
